...
const CardFeed = ({theme}) => {
    ...
    .then(data => {
      setStats([
        ...
        },          
        {
          ...
          mainStat: 
          <div>
            <p className="text-lg font-semibold mb-1"><img className="w-6 h-6 mr-2 inline mb-1" alt="" src={`https://s1.coincarp.com${data.topProjects[0].logo}`}/> {data.topProjects[0]._id+' - '+'$'+formatAmount(parseInt(data.topProjects[0].sum))}</p>
            <p className="text-sm font-medium text-gray-500 mb-1"><img className="w-6 h-6 mr-2 inline mb-1" alt="" src={`https://s1.coincarp.com${data.topProjects[1].logo}`}/>{data.topProjects[1]._id+' - $'+formatAmount(parseInt(data.topProjects[1].sum))}</p>
            <p className="text-xs font-medium text-gray-500 whitespace-nowrap"><img className="w-6 h-6  mr-2 inline mb-1" alt="" src={`https://s1.coincarp.com${data.topProjects[2].logo}`}/> {data.topProjects[2]._id+' - $'+formatAmount(parseInt(data.topProjects[2].sum))}</p>
          </div>,
        },
        {
          ...
          mainStat: 
          <div>
            <p className="text-lg font-semibold mb-1"><img className="w-6 h-6  mr-2 inline mb-1" alt="" src={`https://s1.coincarp.com${data.topInvestors[0].logo}`}/> {data.topInvestors[0]._id}</p>
            <p className="text-sm font-medium text-gray-500 mb-1"><img className="w-6 h-6  mr-2 inline mb-1" alt="" src={`https://s1.coincarp.com${data.topInvestors[1].logo}`}/> {data.topInvestors[1]._id}</p>
            <p className="text-xs font-medium text-gray-500"><img className="w-6 h-6 mr-2 inline mb-1" alt="" src={`https://s1.coincarp.com${data.topInvestors[2].logo}`}/> {data.topInvestors[2]._id}</p>
          </div>
        }]);
      }) 
      .catch(error => {
  ...
  return (
    <div className="mt-5 space-y-3 mx-8">
      <Select styles={customStyles} options={Options} value={timeframe} onChange={handleTimeframeChange} placeholder="This Week"/>
      <div className="grid gap-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 ">
        {stats.map((cardData,index) => (
            <Card id={index} key={index} title={cardData.title} icon={cardData.icon} mainStat={cardData.mainStat} comparisonStat={cardData.comparisonStat} theme={theme} />)
            )
        }
      </div>
    </div>
